...
class AlignmentStreamAnalyzer:
    ...
    def step(self, logits, next_token=None):
        ...
        aligned_attn = torch.stack(
            self.last_aligned_attns).mean(dim=0)  # (N, N)
        ...
        long_tail = self.complete and (
            A[self.completed_at:, -3:].sum(dim=0).max() >= 5)  # 200ms
        ...
        if next_token is not None:
            ...
            if isinstance(next_token, torch.Tensor):
                token_id = next_token.item() if next_token.numel(
                ) == 1 else next_token.view(-1)[0].item()
            else:
                token_id = next_token
        ...
        if token_repetition:
            ...
            logger.warning(
                f"🚨 Detected 2x repetition of token {repeated_token}")
